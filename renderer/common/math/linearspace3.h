...
namespace embree
{
  ...
  template<class T> __forceinline BBox<T> xfmBBox(const LinearSpace3f& s, const BBox<T>& b){
    T corners[8];
    b.GetCorners(corners);
    BBox< T > ret = BBox< T >(EmptyTy());
    for(int i = 0; i < 8; i ++) ret.grow(xfmPoint(s,corners[i]));
    return ret;
  }
}
