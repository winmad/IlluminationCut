...
namespace embree
{
  vplRenderer::vplRenderer(const Parms& parms)
    : iteration(0)
  {
    /*! create integrator to use */
    std::string _integrator = parms.getString("integrator","vplintegrator");
    if (_integrator == "vplintegrator") integrator = new vplIntegrator(parms);
  ...
  void vplRenderer::renderFrame(const Ref<Camera>& camera, const Ref<BackendScene>& scene, const Ref<ToneMapper>& toneMapper, Ref<SwapChain > swapchain, int accumulate) 
  {
  ...
  vplRenderer::RenderJob::RenderJob (Ref<vplRenderer> renderer, const Ref<Camera>& camera, const Ref<BackendScene>& scene, 
  ...
  void vplRenderer::RenderJob::finish(size_t threadIndex, size_t threadCount, TaskScheduler::Event* event)
  {
  ...
  void vplRenderer::RenderJob::renderTile(size_t threadIndex, size_t threadCount, size_t taskIndex, size_t taskCount, TaskScheduler::Event* event)
  {
