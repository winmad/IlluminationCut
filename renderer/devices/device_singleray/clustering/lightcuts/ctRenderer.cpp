...
namespace embree
{
  ctRenderer::ctRenderer(const Parms& parms)
    : iteration(0)
  {
    /*! create integrator to use */
    std::string _integrator = parms.getString("integrator","ctintegrator");
    if (_integrator == "ctintegrator") integrator = new ctIntegrator(parms);
  ...
  void ctRenderer::renderFrame(const Ref<Camera>& camera, const Ref<BackendScene>& scene, const Ref<ToneMapper>& toneMapper, Ref<SwapChain > swapchain, int accumulate) 
  {
    ...
    for (size_t i = 0 ; i < swapchain->getWidth(); ++i)
    {
      ...
      {
        Color L = swapchain->buffer()->get(i, j);
        swapchain->buffer()->set(i, j, L );
      }
    }
#endif
    iteration++;
  ...
  ctRenderer::RenderJob::RenderJob (Ref<ctRenderer> renderer, const Ref<Camera>& camera, const Ref<BackendScene>& scene, 
  ...
  void ctRenderer::RenderJob::finish(size_t threadIndex, size_t threadCount, TaskScheduler::Event* event)
  {
  ...
  void ctRenderer::RenderJob::renderTile(size_t threadIndex, size_t threadCount, size_t taskIndex, size_t taskCount, TaskScheduler::Event* event)
  {
    TIMESTART(stats.atomic.timeTotal,tot0);
    /*! create a new sampler */
    IntegratorState state;
    if (taskIndex == taskCount-1) t0 = getSeconds();

    ctIntegrator* cti = (ctIntegrator*) renderer->integrator.ptr;
    std::vector<CutTriple> cut;cut.reserve(800);
      ...
      for (size_t dy=0; dy<TILE_SIZE; dy++)
      {
        ...
        for (size_t dx=0; dx<TILE_SIZE; dx++)
        {
          ...
          for (size_t s=0; s<spp; s++)
          {
            ...
            L += cti->LiCut(primary, scene, state,cut);
          }
    ...
    TIMESTOP(stats.atomic.timeTotal,tot0);
