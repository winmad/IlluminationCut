...
#ifndef __EMBREE_LS_RENDERER_H__
#define __EMBREE_LS_RENDERER_H__
...
#include "devices/device_singleray/filters/filter.h"
#include "devices/device_singleray/renderers/progress.h"
#include "common/sys/taskscheduler.h"
#include "devices/device_singleray/clustering/lightslice/lsKnnMatrix.h"

namespace embree
{
  /*! Renderer that uses a given integrator, sampler, and pixel
   *  filter. */
  class lsRenderer : public Renderer
  {
  public:

    /*! Construction from parameters. */
    lsRenderer (const Parms& parms);
  ...
  private:
    ...
    {
    public:
      RenderJob (Ref<lsRenderer> renderer, const Ref<Camera>& camera, const Ref<BackendScene>& scene, 
  ...
  public:
    Ref<KnnMatrix> knnMat;
    int seed;                   //!< seed for the lightslices (controls the number of them)
    int budget;                   //!< budget size (number of clusters)
  };
}
