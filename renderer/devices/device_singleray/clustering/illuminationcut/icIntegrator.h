...
#ifndef __EMBREE_IC_INTEGRATOR_H__
#define __EMBREE_IC_INTEGRATOR_H__
...
namespace embree
{
  ...
  class icIntegrator : public Integrator
  {
  ...
  public:
    ...
    icIntegrator(const Parms& parms);

    /*! Registers samples we need from the sampler. */
    ...
    /*! Returns the surface sample hit by the ray, its emitted radiance etc */
    Color getPixel(Ray& ray, const Ref<BackendScene>& scene, IntegratorState& state, DifferentialGeometry& dg,Vector3f& wo,Color& kd, Color& ks, float& exp, bool& hit);
