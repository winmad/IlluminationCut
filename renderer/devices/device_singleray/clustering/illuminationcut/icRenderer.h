...
#ifndef __EMBREE_IC_RENDERER_H__
#define __EMBREE_IC_RENDERER_H__
...
namespace embree
{
  ...
  private:
    ...
    {
    public:
      RenderJob (Ref<icRenderer> renderer, const Ref<Camera>& camera, const Ref<BackendScene>& scene, 
                 const Ref<ToneMapper>& toneMapper, Ref<SwapChain > swapchain, int accumulate, int iteration);
       
    private:

      TASK_RUN_FUNCTION(RenderJob,getPixels);
      TASK_COMPLETE_FUNCTION(RenderJob,donothing);
      void setPixels();
      
      /*! Arguments of renderFrame function */
    private:
      Ref<icRenderer> renderer;
