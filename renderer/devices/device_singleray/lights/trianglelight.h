...
namespace embree
{
  ...
  public:
    ...
    Color samplePhoton(Sample3f& direction,Vector3f& location, const Vec2f& dir, const Vec2f& loc, float sceneradius,Vector3f scenecenter) const
    {
      location = uniformSampleTriangle(loc.x,loc.y,tri->v0,tri->v1,tri->v2);
      direction = uniformSampleHemisphere(dir.x, dir.y, normalize(Ng)); 
      direction.pdf *= 2 * rcp(length(cross(e1,e2))); // dividing by the area of the triangle
      return L * clamp(dot(normalize(Ng),direction.value))  ;
    }
