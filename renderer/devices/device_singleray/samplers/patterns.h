...
namespace embree
{
  ...
  /*! Create a set of n stratified samples */
  void stratified2D(Vec2f* samples, const uint32 n, Random& rng,int spp)
  {
    uint32 w = (uint32)sqrtf(float(spp)+0.5f);
    uint32 h = w;
    for(uint32 i = 0, c = 0; i < w; i ++) 
      for(uint32 j = 0; j < h; j ++, c++) 
        samples[c] = Vec2f( (0.5f + i) / w, (0.5f + j) / h);
  }
}
