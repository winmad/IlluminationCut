...
namespace embree
{
  ...
  public:
    ...
    /* HACK to get diff and spec coefficients, very error prone, e.g fails if there is a microfacet brdf*/
    void getPhong(Color& kd, Color& ks, float& exp)
    {
      kd = ks = Color(zero);
      exp = 0.0f;
      for (size_t i = 0 ; i < this->numBRDFs; i++)
      {
        if (this->BRDFs[i]->type == GLOSSY_REFLECTION)
        {
          Specular* spec = (Specular*) this->BRDFs[i];
          ks = spec->R;
          exp = spec->exp;
        }
        else if (this->BRDFs[i]->type == DIFFUSE_REFLECTION)
        {
          Lambertian* lamb = (Lambertian*) this->BRDFs[i];
          kd = lamb->R;
        }else // for unknown material we just sample it perpendicular to the surface
          throw std::runtime_error("Not supported BRDF type");
      }
    }
